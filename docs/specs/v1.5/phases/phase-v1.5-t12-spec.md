...
| REQ-1.5-17 | 保持 v1.1 ~ v1.4 已有能力零回归 | 回归测试 8 个测试文件全绿，无新增失败用例 |
...
| custom-rules | `tests/custom-rules.test.ts` | v1.4 | 确保自定义规则扫描无回归 |
| analyzer | `tests/analyzer.test.ts` | v1.4 | 确保日志聚合分析无回归 |
...
- **不达标处理**：若性能不达标，在设计文档已预留升级为 v1.6 引入 Worker 线程的方案；T12 阶段记录实际耗时数据，在验收报告中标注
...
详见 §3.2，共 6 个测试文件、67 个测试用例（resolver 11 + collector 14 + graph 10 + analyzer 10 + monorepo 13 + scanner 12 = 70，实际以代码为准）。
...
详见 §3.4，共 8 个既有测试文件，要求 `pnpm test` 全量通过，无新增失败用例。
